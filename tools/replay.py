...
def _flip_signal(sig):
    ...
    цели зеркалятся относительно входа, цель — та же, что у исходного
    сигнала. Меняется ТОЛЬКО сторона — отбор, гейты и моменты входа
    остаются боевыми.

    Кратность берётся из САМОГО сигнала (поле rr), а не из константы: цель
    стала настраиваемой (cfg.TP_R_MULT), и зашитая двойка сравнивала бы
    развёрнутую сделку с 2R против боевой с другой целью — разница тогда
    меряла бы геометрию, а не знак направления. Для истории, собранной при
    2R, значение равно 2.0 и замер IV воспроизводится в точности.
    ...
    tp_r = float(getattr(sig, "rr", 0.0) or 0.0)
    if tp_r <= 0:
        # Сигнал без записанной кратности: 2.0 — геометрия, при которой
        # собрана вся имеющаяся история.
        tp_r = 2.0
    f.sl = entry - s * risk
    f.tp1 = entry + s * tp_r * 0.5 * risk
    f.tp2 = entry + s * tp_r * risk
    f.tp3 = entry + s * tp_r * 1.5 * risk
    return f
