...
# Насколько старой может быть последняя свеча, чтобы цена считалась
# действующей. Монета, не торговавшаяся сутки, не торгуется.
_MAX_STALE_MS = 24 * 3600 * 1000


def _price_at(k4: List[Dict], now_ms: int) -> Optional[float]:
    """Цена на момент now_ms — или None, если ряд к этому моменту оборвался.

    Без проверки свежести возвращалась последняя известная цена ЛЮБОЙ
    давности, и символ, переставший торговаться в середине недели, давал
    доходность ровно 0.0 (застывшая цена, делённая сама на себя) вместо
    пропуска. Ноль в шортовой ноге — это подарок: делистятся преимущественно
    слабейшие, то есть ровно те имена, где шорт должен был заработать или
    потерять по-настоящему.

    Та же поправка уже внесена в tools/xsec_universe.py (замер V-бис), где
    делистинг обрабатывается явно; здесь она отсутствовала, и запуск
    старого замера продолжал бы давать смещение в оптимистичную сторону.
    """
    closed = _closed_upto(k4, now_ms)
    if not closed:
        return None
    last = closed[-1]
    if now_ms - last["ts"] > _MAX_STALE_MS:
        return None
    px = last["close"]
