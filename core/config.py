...
class Config:
    ...
    #
    # 100, а не 500 — из-за ТРАФИКА, а не числа запросов. Ответ
    # recent-trade весит ~141 байт на сделку, и при глубине 500 скан
    # тяжелеет в 22 раза: 7.4 МБ на скан, 2.65 ГБ в сутки. Прокси Webshare
    # тарифицируются по трафику, и стартовый гигабайт выгорал бы за 9
    # часов. Дальше цепочка механическая: прокси отдают 402/403 -> перебор
    # -> прямое соединение с IP Railway -> гео-блок Bybit ->
    # get_positions() = None -> НЕПРЕРЫВНАЯ проверка наличия стопа
    # прекращается. То есть рецидивирующий баг №1, вызванный квотой на
    # трафик. При 100 это 0.62 ГБ/сутки, при 50 — 0.37 ГБ/сутки.
    # Глубина ленты сделок. 1000 — потолок Bybit, и он выбран НЕ с запасом,
    # а по замеру: при лимите 100 лента покрывала минимум минуту лишь у 46%
    # сигналов, при 500 — у 80%, при 1000 — у 89%.
    #
    # Почему это критично. Сто сделок покрывают тем МЕНЬШЕ времени, чем
    # сильнее всплеск активности — то есть ровно на объёмном климаксе,
    # который стратегия и ловит. Отбраковка «короткая лента» оказалась не
    # случайной, а систематической: у отброшенных ATR 9.9% против 4.1% у
    # оставшихся, score 46.8 против 35.8, стоп 11.8% против 4.7%. Замер
    # потока строился на самых спокойных сигналах — из 25 пригодных
    # торгуемыми по score были ПЯТЬ.
