...
async def cleanup_old_signals(keep_hours: int = 192) -> int:
    ...
    try:
        async with aiosqlite.connect(DB_PATH) as db:
            ...
            # Потолок по числу НЕ ТРОГАЕТ сигналы, которые ещё можно
            # досудить.
            #
            # В первой редакции этой правки условия на возраст не было, и
            # потолок сносил нерешённые ВНУТРИ окна оценки: на прогоне из
            # 20 свежих сигналов при потолке 10 удалялись все возрастом
            # 10-19 часов при окне 48. Стопы (1R) разрешаются быстрее целей
            # (2R), поэтому удаляются преимущественно будущие ПОБЕДЫ, и
            # винрейт занижается систематически — ровно та ошибка, от
            # которой предостерегает комментарий к лимиту строк выше.
            #
            # Поэтому потолок применяется только к тем, до кого оценщик уже
            # не дотянется: старше окна оценки. Рост при этом ограничен —
            # худший случай ровно окно оценки сигналов.
            judge_cutoff = (datetime.utcnow()
                            - timedelta(hours=_JUDGE_WINDOW_HOURS)).isoformat()
                ...
                """DELETE FROM signals
                   WHERE outcome IS NULL AND ts < ?
                     AND id NOT IN (SELECT id FROM signals
                                    WHERE outcome IS NULL AND ts < ?
                                    ORDER BY ts DESC LIMIT ?)""",
                (judge_cutoff, judge_cutoff, max(cfg.MAX_SIGNALS_DB, 1)),
            )
