# gerchik-bot

Бот-сканер сигналов и автотрейдер для Bybit (V5 API, USDT linear perps).
FastAPI + uvicorn на Railway, дашборд в static/index.html, БД — aiosqlite.

## Структура
- `strategy/scanner.py` — скоринг-движок: VSA + ключевые уровни (swing) + MTF-фильтр + MIN_RR;
  _direction (voting + confidence) → _apply_confluence_cap. Confluence-капы удалять НЕЛЬЗЯ.
- `strategy/trader.py` — вход/мониторинг позиций, риск-гарды
- `core/config.py` — все параметры (env vars)
- `core/state.py` — in-memory состояние (Signal, Position, state singleton)
- `core/db.py` — SQLite (signals, trades)
- `exchange/bybit.py` — async REST клиент Bybit V5
- `api/routes.py` — FastAPI endpoints + WebSocket
- `static/index.html` — весь фронтенд (дашборд)

## Известные рецидивирующие баги — ВСЕГДА проверять
- SL/TP исторически ставились только как чарт-маркеры, реально не долетая
  до биржи. После любого изменения в enter_trade/place_order — обязательно
  проверять, что SL/TP подтверждены через live-позицию с биржи, а не только
  что retCode==0.
- Score и direction раньше считались независимо друг от друга — при любых
  правках scoring engine проверять согласованность (confluence) факторов.

## Инварианты, которые нельзя нарушать
- Риск на сделку: 1–3% от баланса (дефолт 1%, потолок 3% — жёсткий)
- Плечо: максимум 3-5x
- SL всегда обязателен перед входом — без исключений
- Не торговать новостные/листинговые спайки

## Ревью кода
- Перед любой проверкой (своей или агентами) читать `docs/REVIEW.md`:
  эталонные параметры, связи между ними, роли для группы агентов.
  Разделы 0-А и 0-Б — обязательная дисциплина, они собраны из уже
  допущенных ошибок; пропуск любого пункта эти ошибки возвращает.
- `docs/LITERATURE.md` — внешняя опора по стратегии (уровни, объём,
  поток ордеров, фандинг, переобучение, размер выборки). Утверждение
  «фактор работает» принимается только с замером на наших данных или
  ссылкой оттуда; «подтверждено исследованиями» без ссылки — не аргумент.
  Предлагая менять порог, назвать, сколько его вариантов уже проверено
  на той же выборке (множественная проверка, §6).
- Агентов запускать по ролям из брифа (стратегия / деньги / данные /
  интерфейс) — иначе дублируют работу и пропускают стыки между слоями.
- Дерево должно быть закоммичено ДО запуска агентов: агент с
  `git checkout --` стирает незакоммиченные правки (уже случалось).
- Отчёт агента — гипотеза. Цитируемый код перепроверять самому.

## Перед тем как считать задачу выполненной
- `python3 -m pytest tests/ -q` — зелёный (набор фиксирует все прошлые баги)
- `python3 -m mypy . --ignore-missing-imports` — чисто
- Проверить синтаксис (python3 -m py_compile)
- Новый тест проверить мутацией: сломать охраняемую строку и убедиться,
  что тест падает. Не падает — теста нет, есть его иллюзия.
- Пройтись по cross-file зависимостям (scanner.py → trader.py → config.py)
- Явно перечислить: что могло сломаться в других местах от этого изменения

## Git
- Пушить в обе ветки: `git push origin main:claude/pepeto-JQ02Z` и `git push origin main`
