...
async def _delayed_initial_scan():
    ...
    # СВЕРКА ДОЛЖНА СОСТОЯТЬСЯ, а не просто не бросить. monitor_positions
    # глушит свои исключения и ШТАТНО возвращается, ничего не заполнив,
    # когда get_positions() отдала None (сбой API, таймаут, 403). Признака
    # успеха у неё нет, поэтому except выше почти мёртв.
    #
    # Чем это кончалось: рестарт при открытых позициях, первый
    # get_positions отвалился → state.positions пуст → скан находит сигнал →
    # enter_trade видит 0 позиций из MAX_POSITIONS и открывает ЛИШНЮЮ,
    # поверх уже существующих. MAX_SAME_DIRECTION считается от нуля так же.
    # enter_trade нигде не перезапрашивает позиции с биржи.
    #
    # Спрашиваем биржу сами: None значит «не знаю», и в этом случае первый
    # скан пропускается — монитор повторит через 30 секунд, и скан пойдёт
    # по расписанию. Потерять один скан дешевле, чем открыть лишнюю позицию.
    try:
        live = await _client.get_positions()
    except Exception as e:
        live = None
        log.error(f"Стартовая сверка: запрос позиций не прошёл — {e}")
    if live is None:
        log.error("Стартовый скан ПРОПУЩЕН: состав позиций неизвестен, "
                  "лимиты считались бы от нуля и бот открыл бы лишнюю "
                  "позицию поверх существующих")
        return
    try:
        await _scan_job()
    except Exception as e:
