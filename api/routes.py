...
async def get_stats(request: Request):
    ...
    return JSONResponse(_sanitize({
        ...
        # Встречное число к обнулённой статистике: сколько исходов лежит под
        # ДРУГИМ ярлыком. Без него «ноль» от смены ярлыка неотличим от
        # «ноль» от отсутствия данных.
        "foreign_strategy_rows": await db.foreign_strategy_rows(),
        ...
        "trading_halted": state.trading_halted,
        # Причина остановки: «дневной лимит» и «база не поднялась» требуют
        # РАЗНЫХ действий, а на экране выглядели одинаково.
        "halt_reason": state.halt_reason or None,
        # Позиции с риском выше жёсткого потолка 3%. Обнаружение жило
        # только в логе одной строкой за жизнь позиции — при отсутствии
        # алертинга это ноль внимания.
        "over_risk": dict(state.over_risk),
        ...
        # Ключ был ЗАДВОЕН: этот, вычисляемый по дате, стоял ниже и
        # побеждал — Python оставляет последнее значение. Причина при этом
        # выводилась ложно: _ensure_daily_state штампует сегодняшнюю дату,
        # даже когда УДЕРЖИВАЕТ халт по неисправной базе, и на экран шло
        # «дневной лимит убытка достигнут (0.00 USDT)». Владелец ждал бы
        # полуночи, а халт по такой причине ролловером не снимается —
        # то есть бессрочно. Причину теперь несёт state.halt_reason выше.
        "daily_realized_pnl": round(state.daily_realized_pnl, 2),
    }))
