...
# Частота поднята с 10 до 20 запросов в секунду, слотов с 6 до 12.
# Причина не в нетерпении: полный набор — 864 символа × 36 месяцев × 2
# ряда ≈ 62 тыс. запросов, и при 10/с это 1.7 часа непрерывной работы.
# Процесс не переживает простой между сессиями, поэтому загрузка
# обязана укладываться в одну. Источник — статика на CDN
# (data.binance.vision), не торговый API: лимитов ордеров тут нет, а
# 404 на несуществующий месяц дешевле самого файла.
