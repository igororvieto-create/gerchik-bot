...
def _mfe(direction: str, entry: float, sl: float, klines: list) -> float:
    ...
    """
    # Направление проверяется ЯВНО, как и в _judge рядом: там защита стоит
    # с формулировкой «раньше всё, кроме LONG, молча трактовалось как
    # SHORT». Здесь её не было, и мусорное направление считалось по шортовой
    # формуле — портя колонку mfe_r, ту самую, по которой меряют пороги.
        ...
        for row in pending:
            try:
                ...
                if (relevant and klines and klines[0]["ts"] > sig_ms
                    ...
                    continue
                # Начало окна НЕ ПОКРЫТО, но окно ещё восстановимо: ждём
                # данные, а не судим по обрубку.
                #
                # Ветка выше закрывает такой сигнал как EXPIRED только когда
                # окно невосстановимо. Если условие в скобках ложно
                # (возраст < 48ч, свечей < 200), выполнение проваливалось
                # ДАЛЬШЕ и выносило вердикт ровно по тому неполному окну,
                # которое комментарий выше объявил недопустимым: неизвестно,
                # был ли стоп задет раньше, а значит винрейт форвард-теста
                # завышается — и это единственное основание включать
                # автоторговлю.
                if relevant and klines and klines[0]["ts"] > sig_ms:
                    log.info(
                        f"evaluate {row['symbol']}: свечи начинаются позже "
                        f"сигнала (возраст {age_h:.0f}ч) — вердикт отложен "
                        f"до появления начала окна")
                    continue
                if not relevant:
