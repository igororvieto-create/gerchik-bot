...
def _direction(sig_type: str, price_change: float, ob_bias: str, funding: float,
    ...
    if sig_type.startswith("VSA_") and vsa_bias != "NEUTRAL":
        ...
        if long_votes > short_votes:
            ...
            derived_from_votes = True
    elif sig_type == "FUNDING_EXTREME" and cfg.FUNDING_VOTE:
        ...
        # Ветка исполняется ТОЛЬКО при включённом голосе фандинга.
        #
        # Раньше условия `and cfg.FUNDING_VOTE` здесь не было: при
        # выключенном флаге фандинг голоса не подавал, но сторону задавал
        # по-прежнему, а снятие «тавтологического» голоса ниже удаляло ОДИН
        # голос, равный primary, — то есть ЧУЖОЙ голос цены или стакана.
        # Вариант «фандинг не голосует» на деле означал «фандинг всё так же
        # выбирает сторону, плюс у сигнала отнят честный голос и ужесточён
        # кап», и сравнение вариантов nofunding/both/strict_nf между собой
        # было недействительно.
        #
        # При дефолте (флаг включён) поведение не меняется.
        derived_from_votes = True
...
def _calc_levels(price: float, atr: float, direction: str,
    ...
    либо, если уровня нет, на 1.5×ATR как раньше.

    Цели строятся ОТ cfg.TP_R_MULT (по умолчанию 2.0): tp1 = 0.5×цели,
    tp2 = цель, tp3 = 1.5×цели. Зашитая сетка 1R/2R/3R, которая тут была
    описана, стала настраиваемой — описание отставало от кода.

    Если до противоположного уровня (цели) реально достижимый R:R < cfg.MIN_RR
    (1.5, а не 3:1 — порог тоже настраиваемый) — сигнал отбраковывается
    (возвращает None): цель, лежащая за встречным уровнем, недостижима.
    """
    if price <= 0 or atr <= 0:
        return None
...
async def _analyze_symbol(client: BybitClient, ticker: dict) -> Optional[Signal]:
    ...
    try:
        ...
        # ОГРАНИЧИТЕЛИ РАЗВОРОТА применяются ко ВСЕМ разворотам, а не
        # только к освобождённым от гейта уровня.
        #
        # Раньше они жили внутри ветки else самого освобождения, и
        # выключение VSA_LEVEL_EXEMPT снимало вместе с гейтом уровня ещё
        # два чужих ограничителя — снос и запрет входа за экстремум. То
        # есть «строгий» вариант оказывался не строже, а иначе дырявее, и
        # варианты strict/strict_nf в прошлых замерах меряли не то, что
        # заявлено. REVIEW §2 прямо требует у разворота собственного
        # ограничителя.
        #
        # При дефолтных настройках поведение НЕ меняется: освобождённый
        # разворот как получал эти проверки, так и получает.
        if is_vsa_reversal:
            if atr <= 0:
                return None
            ...
            # тот же вход вдогонку, только в обратную сторону.
            _sig_c = klines[-2]
            if abs(price - _sig_c["close"]) / atr > cfg.REVERSAL_MAX_DRIFT_ATR:
                return None
            ...
            if direction == "LONG" and price <= _sig_c["low"]:
                return None
            if direction == "SHORT" and price >= _sig_c["high"]:
                return None
