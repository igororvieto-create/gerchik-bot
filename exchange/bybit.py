import hashlib
import hmac
import logging
import time
from typing import Any, Dict, List, Optional

import aiohttp

log = logging.getLogger("bybit")

BASE_URL = "https://api.bybit.com"
RECV_WINDOW = 5000


class BybitClient:
    def __init__(self, api_key: str = "", secret: str = ""):
        self.api_key = api_key
        self.secret = secret
        self._session: Optional[aiohttp.ClientSession] = None

    async def _get_session(self) -> aiohttp.ClientSession:
        if self._session is None or self._session.closed:
            self._session = aiohttp.ClientSession()
        return self._session

    async def close(self) -> None:
        if self._session and not self._session.closed:
            await self._session.close()

    def _auth_headers(self, params_str: str = "") -> Dict[str, str]:
        ts = str(int(time.time() * 1000))
        sign_payload = f"{ts}{self.api_key}{RECV_WINDOW}{params_str}"
        sig = hmac.new(self.secret.encode(), sign_payload.encode(), hashlib.sha256).hexdigest()
        return {
            "X-BAPI-API-KEY":     self.api_key,
            "X-BAPI-SIGN":        sig,
            "X-BAPI-SIGN-TYPE":   "2",
            "X-BAPI-TIMESTAMP":   ts,
            "X-BAPI-RECV-WINDOW": str(RECV_WINDOW),
            "Content-Type":       "application/json",
        }

    async def _get(self, path: str, params: Dict = None, auth: bool = False) -> Dict:
        session = await self._get_session()
        params = params or {}
        query = "&".join(f"{k}={v}" for k, v in sorted(params.items()))
        headers = self._auth_headers(query) if auth else {}
        url = BASE_URL + path + (f"?{query}" if query else "")
        for attempt in range(3):
            try:
                async with session.get(url, headers=headers, timeout=aiohttp.ClientTimeout(total=10)) as r:
                    data = await r.json()
                    if data.get("retCode", 0) != 0:
                        log.warning(f"GET {path} -> retCode {data.get('retCode')}: {data.get('retMsg')}")
                    return data
            except Exception as e:
                if attempt == 2:
                    log.error(f"GET {path} failed after 3 attempts: {e}")
                    return {}
                await __import__("asyncio").sleep(1)
        return {}

    # ── Public market data ────────────────────────────────────────────────────

    async def get_tickers(self) -> List[Dict]:
        """All linear USDT perp tickers — price, volume, funding, OI in one call."""
        data = await self._get("/v5/market/tickers", {"category": "linear"})
        return data.get("result", {}).get("list", [])

    async def get_klines(self, symbol: str, interval: str = "240", limit: int = 25) -> List[Dict]:
        """OHLCV candles. interval: '1','3','5','15','30','60','120','240','D','W'."""
        data = await self._get("/v5/market/kline", {
            "category": "linear",
            "symbol":   symbol,
            "interval": interval,
            "limit":    limit,
        })
        raw = data.get("result", {}).get("list", [])
        # Bybit returns newest first; reverse to oldest-first
        return [
            {
                "ts":     int(r[0]),
                "open":   float(r[1]),
                "high":   float(r[2]),
                "low":    float(r[3]),
                "close":  float(r[4]),
                "volume": float(r[5]),
            }
            for r in reversed(raw)
        ]

    async def get_open_interest(self, symbol: str, interval: str = "4h", limit: int = 12) -> List[Dict]:
        """OI history for a single symbol."""
        data = await self._get("/v5/market/open-interest", {
            "category":     "linear",
            "symbol":       symbol,
            "intervalTime": interval,
            "limit":        limit,
        })
        raw = data.get("result", {}).get("list", [])
        return [{"ts": int(r["timestamp"]), "oi": float(r["openInterest"])} for r in raw]

    async def get_orderbook(self, symbol: str, limit: int = 20) -> Dict:
        """Level-2 orderbook snapshot."""
        data = await self._get("/v5/market/orderbook", {
            "category": "linear",
            "symbol":   symbol,
            "limit":    limit,
        })
        result = data.get("result", {})
        bids = [[float(p), float(q)] for p, q in result.get("b", [])]
        asks = [[float(p), float(q)] for p, q in result.get("a", [])]
        return {"bids": bids, "asks": asks}
