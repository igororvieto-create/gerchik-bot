...
class Config:
    ...
    #
    # ДЕФОЛТ 0 С 2026-09-25: замер III закрыт, фактор не прошёл планку ни по
    # одному из четырёх условий (docs/FLOW.md). Лента запрашивалась ТОЛЬКО
    # ради него, и дальше это чистые издержки — тот самый трафик, который
    # выжигает квоту прокси и по цепочке приводит к рецидивирующему багу №1.
    #
    # Код сбора сохранён намеренно: удаление машинерии уничтожило бы
    # возможность воспроизвести состоявшееся измерение. Поднять значение
    # выше нуля = снова собирать ленту; делать это стоит только под НОВУЮ
    # пре-регистрированную гипотезу, а не «посмотреть ещё раз».
    TRADE_FLOW_LIMIT: int = _env_int("TRADE_FLOW_LIMIT", 0)
